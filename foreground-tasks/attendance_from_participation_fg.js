...
const filterBlacklistedGradeTypes = (g) => {
    return Object.hasOwn(g, 'GradeType') && !['Calculated', 'Text'].includes(g.GradeType);
};
...
const getGradeItems = (ou) => {
    return fetch(`/d2l/api/le/1.34/${ou}/grades/categories/`)
        ...
        .then((categories) => {
            ...
            return fetch(`/d2l/api/le/1.34/${ou}/grades/`).then((res) => {
                return res.json();
            }).then((gItems) => {
                const noCategory = gItems.filter((e) => {
                    return e.CategoryId === 0 && filterBlacklistedGradeTypes(e);
                });
...
const getStudents = (ou, gradeItems) => {
    return Promise.all(gradeItems.map((g) => {
...
class WindowBuilder {
    ...
    buildCheckList() {
        ...
        fillAbsentButton.insertAfter(afterPoint);
        ...
        processButton.insertAfter(afterPoint);
